...
mo=load('proj\7 2 中油 iron dic and pull\LOG46.txt');
...
%% power
close all
powerdata=xlsread('proj/7 2 中油 iron dic and pull/POWER4.CSV');
...
% pt0=0 +151.5340; %拉3
% pt0=0 +204.0670; %拉4
% pt0=0 +668.3400; %5 5iron
ponmin=[2 6 10 14 19 23 31 33 35 37]; %pull1
ponsec=[38 48 17 35 5 27 21 13 35 43];
poffmin=[4 8 12 17 22 26 32 34 36 38];
poffsec=[33 40 20 49 4 38 17 10 16 20];
% ponmin=[0 4 8 10  14 18 22 24 25 27]; %pull2
% ponsec=[36 35 6 42 55 18 17 22 55 28];
% poffmin=[2 6 9 13 17 21 23 25 26 28];
% poffsec=[35 34 45 15 18 27 18 10 40 5];
% ponmin=[18 25 28 31 34 40 45 47 48 50]; %pull3
% ponsec=[20 11 50 13 34 52 33 15 59 34];
% poffmin=[19 26 30 33 37 43 46 48 49 51];
% poffsec=[55 51 15 42 4 57 17 0 38 10];
% ponmin=[1 5 8 10 15 19 24 26 28 31]; %pull4
% ponsec=[28 14 9 43 8 52 45 31 25 15];
% poffmin=[3 6 9 13 17 23 25 27 29 32];
% poffsec=[15 55 40 50 47 3 28 10 14 2];
...
[~,k]=size(ponmin);
for i=1:k
pon(i)=ponmin(i)*60+ponsec(i)+pt0; %%%%%%%%%%%
poff(i)=poffmin(i)*60+poffsec(i)+pt0;
end
[~,k]=size(pon);
pton(k,:)=0;
ptoff(k,:)=0;
...
xlswrite('test.xlsx',text,pulln)
xlswrite('test.xlsx',pulldata,pulln,'A2')
end
%% hall %%下多似計步器推算
addpath('hall');
...
% on=onn+152;%5 11 對稱
on=[t(11800) t(16600) t(21800) t(27500)]; %% 6 19 magbox d compare
off=on-3;
